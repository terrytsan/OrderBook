...
class OrderDetails extends Component {
  ...
  render() {
    ...
    let latestOrder = orderRecords.reduce((max, order) => max.version > order.version ? max : order);
    let firstOrder = orderRecords.reduce((max, order) => max.version < order.version ? max : order);
    ...
    return (
      <div className="container">
        ...
        <OrderBreakDown order={order} latestOrder={latestOrder} firstOrder={firstOrder} totalQuantity={totalQuantity}/>
        <OrderHistoryTable
          order={order}
